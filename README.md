...
|  | (LiteLLM Embedding API   |  |  (Claims table with    |  |  (Validated Pydantic   |  |
...
5. Tools execute: RAG query to pgvector, claim lookup in Postgres, or claim submission in Postgres
...
4. **Open the Application**
   Navigate to [http://localhost:3000](http://localhost:3000) in your browser. The backend will automatically ingest the policy document into pgvector on startup.
...
|   +-- requirements.txt            # Python dependencies (fastapi, google-adk, litellm, pgvector)
|   +-- app/
...
|   |       +-- policy_rag.py   # query_policy tool (queries pgvector)
...
|       |   +-- ingest.py           # Markdown chunker & pgvector indexing script
...
5. The model can return a plain text answer, or emit `function_calls` for:
   - `query_policy` → RAG search over pgvector policy chunks
